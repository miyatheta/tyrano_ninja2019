...
;疲労の反映
[if exp="f.P_EXH >79"]疲労（重度）[p][eval exp="tf.P_AGI=tf.P_AGI-20 , tf.P_ACTmax=tf.P_ACTmax-2"]
[elsif exp="f.P_EXH >59"]疲労[p][eval exp="tf.P_AGI=tf.P_AGI-10 , tf.P_ACTmax=tf.P_ACTmax-1"]
[endif]
...
[eval exp="tf.Max=99 , tf.Min=0"][dice][jump target="*P_attack_miss" cond="tf.HitRate < tf.dice"]
[if exp="tf.E_AVD>0"][eval exp="tf.Max=99 , tf.Min=1"][dice][jump target="*E_avoid_success" cond="tf.AvoidRate > tf.dice"][endif]
[Calc_Damage]
...
[eval exp="tf.Max=4 , tf.Min=0"][dice][eval exp="tf.loop=tf.dice"]
[jump target="*E_Attack_select"]
[s]
...
*P_DEF_conf1
[jump target="*P_Def_select" cond="tf.P_ACT <= 0"]
[Calc_Status]
[eval exp="tf.P_AVD=Math.floor(tf.P_AGI * tf.ArousAGId * 3) , tf.P_GRD=0"]
[eval exp="tf.AvoidRate = 100 - (tf.HIT - tf.P_AVD)"][limit]
