...
[eval exp="tf.P_DEFx1=0 , tf.P_AGIx1=0"]
[eval exp="tf.E_HP=f.E_HP , tf.E_STR=f.E_STR , tf.E_DUR=f.E_DUR , tf.E_AGI=f.E_AGI , tf.E_ACT=f.E_ACT"]
[eval exp="tf.E_DEFx1=0 , tf.E_AGIx1=0"]

[eval exp="tf.E_AGI=tf.E_AGI + 1"]
...
[eval exp="tf.TAG = tf.ACC + Math.floor(tf.P_AGI - tf.E_AGI * (1 - tf.E_AGIx1)) * 5"]
...
[eval exp="tf.TAG = tf.ACC + Math.floor(tf.P_AGI - tf.E_AGI * (1 - tf.E_AGIx1)) * 5"]
くない：威力28×３：命中率[emb exp="tf.TAG"]％：呼吸--：手番終了[p]
...
[else]
;回避
[eval exp="tf.E_AVD = tf.E_AGI + 1 , tf.E_GRD=1 , tf.E_ACT=tf.E_ACT-1"]
...
[eval exp="tf.Max=9 , tf.Min=0+f.E_LUK"][dice][eval exp="tf.DEF = (tf.E_DUR * tf.E_GRD * 2 + tf.dice) * (1 - tf.E_DEFx1)"]
...
[eval exp="tf.E_DEFx1 = tf.E_DEFx1 + 0.1" cond="tf.E_DEFx1 <= 1"]
...
[eval exp="tf.E_AGIx1 = tf.E_AGIx1 + 0.1" cond="tf.E_AGIx1 <= 1"]
敵に[emb exp="tf.Damage"]のダメージ[p]
敵の回避と命中が低下[p]
[triage]
[jump target="*P_turn_start"]
...
[eval exp="tf.TAG = tf.ACC + Math.floor(tf.E_AGI - tf.P_AGI * (1 - tf.P_AGIx1)) * 5"]
...
[eval exp="tf.Max=9 , tf.Min=0+f.P_LUK"][dice][eval exp="tf.DEF = (tf.P_DUR * tf.P_GRD * 2 + tf.dice) * (1 - tf.P_DEFx1)"]
