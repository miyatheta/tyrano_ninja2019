...
[eval exp="tf.Max=9 , tf.Min=0+f.P_LUK"][eval exp="tf.ATP = (tf.P_POW * tf.OrgaPOWb) * 18 + tf.dice"]
...
[call target="*E_Def_select"]
[eval exp="tf.DEF = Math.floor(tf.E_DUR * tf.E_GRD * 2)"]
[eval exp="tf.Max=9 , tf.Min=0+f.P_LUK"][eval exp="tf.ATP = (tf.P_POW * tf.OrgaPOWb) * 30 + tf.dice"]
