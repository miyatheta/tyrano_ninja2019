...
if(tf.dice  < tf.rate){f.EnCRI = 1.4}
[endscript]
[endmacro]
...
tf.ATP = Math.floor(f.STR * 3 * f.GBuff * f.CRI * tf.randomNum);
tf.EnDFP = Math.floor(f.EnDEF * 1);
...
tf.EnATP = Math.floor(f.EnSTR * 3 * f.EnGBuff * tf.DeBuff * f.EnCRI * tf.randomNum);
tf.DFP = Math.floor(f.DEF * 1);
...
tf.ATP = 100 + Math.floor(f.STR * 2.25 * tf.randomNum);
